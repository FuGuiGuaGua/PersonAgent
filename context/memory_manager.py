...
class MemoryManager:
    ...
    def get_context_for_agent(self, long_term_summary: str = None) -> str:
        ...
        # 使用if v过滤掉空字符串、None、False、0等假值；使用any来保证至少存在一个有效值
        ...
        # 短期记忆（当前会话）
        # fixme 注意：这里的短期记忆只保留了当前记忆的最近3轮，如果当前会话超过3轮，之前的记忆就被丢掉了，
        # fixme 而且长期记忆只保留了和当前session_id不同,也就是之前轮的会话记忆总结
    ...
    #note 这里只总结了除了当前会话之前的会话，当前会话没有被写进记忆总结
